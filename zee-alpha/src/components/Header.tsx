...
export function Header({ onNavigate, currentPage = 'home' }: HeaderProps) {
  ...
  const navItems = [
    ...
    { label: 'About Us', type: 'dropdown', items: [
      { label: 'Overview', action: () => handleSectionClick('about') },
      { label: 'Why Choose Us', action: () => handleSectionClick('why-choose-us') }
    ]},
    { label: 'Admissions', action: () => handleSectionClick('admissions'), type: 'button' },
    { label: 'School Life', type: 'dropdown', items: [
      { label: 'Overview', action: () => handleSectionClick('school-life') },
      { label: 'Gallery Dashboard', action: () => handleNavigation('gallery-dashboard') }
    ]},
    { label: 'Contact', action: () => handleSectionClick('contact'), type: 'button' }
  ];
          ...
          >
            <div className="flex items-center gap-3">
              <div className="relative">
                <img src="/images/school logo.jpeg" className="relative w-16 h-16 rounded-full object-cover border-2 border-white shadow-lg" alt="Logo"/>
              </div>
              <div className="hidden sm:block">
                <h1 className="text-2xl font-bold text-red-700">
                  ZEE-ALPHA
                </h1>
                <p className="text-xs text-gray-500 font-medium tracking-wide">Nurturing Future Leaders</p>  
              </div>
            </div>
          </div>
